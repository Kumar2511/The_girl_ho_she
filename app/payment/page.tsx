...
export default function PaymentPage() {
  ...
  const [upiProofValidated, setUPIProofValidated] =
    useState(false);

  const [upiProofValidationLoading, setUPIProofValidationLoading] =
    useState(false);

  // ==========================================
  // UPI 5-MINUTE PAYMENT SESSION
  // IMPORTANT: this is NOT a MongoDB order.
  // ==========================================

  const [paymentExpiresAt, setPaymentExpiresAt] =
    useState<string | null>(null);

  const [paymentSecondsLeft, setPaymentSecondsLeft] =
    useState(0);

  // ==========================================
// START 5-MINUTE UPI PAYMENT SESSION
// IMPORTANT: DO NOT CREATE AN ORDER HERE.
// ==========================================

const startUPIPaymentSession = () => {
  if (!checkoutData || loading) {
    return;
  }

  if (!upiSettings?.enabled) {
    setUPIError("UPI payment is currently unavailable.");
    return;
  }

  const existingExpiry =
    localStorage.getItem("upiPaymentExpiresAt");
  ...
  if (existingExpiry) {
    ...
    if (remaining > 0) {
      setPaymentExpiresAt(existingExpiry);
      setPaymentSecondsLeft(Math.ceil(remaining / 1000));
      setPaymentMethod("UPI");
      return;
    }

    localStorage.removeItem("upiPaymentExpiresAt");
  }
...
// ==========================================
// RESTORE 5-MINUTE UPI SESSION
// IMPORTANT: no order lookup and no order cancellation.
// ==========================================
...
useEffect(() => {
  ...
  setPaymentExpiresAt(savedExpiry);
  setPaymentSecondsLeft(Math.ceil(remaining / 1000));
}, []);

// ==========================================
  // 5-MINUTE PAYMENT COUNTDOWN
  // When the session expires, return to checkout.
  // No alert and no order is created/cancelled here.
  // ==========================================
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const remaining = Math.max(
        0,
        Math.ceil(
          (new Date(paymentExpiresAt).getTime() - Date.now()) / 1000
        )
      );
      ...
      setUPIScreenshotPreview("");
      setUPIProofValidated(false);
      setUPIProofValidationLoading(false);
      setUPIError("");
  ...
  }, [paymentExpiresAt, router, upiScreenshotPreview]);

  const [dynamicUPIQR, setDynamicUPIQR] =
    useState("");
  ...
  // ==========================================
  // DYNAMIC UPI QR WITH EXACT ORDER AMOUNT
  // ==========================================
  ...
  useEffect(() => {
    ...
    const generateDynamicUPIQR = async () => {
      ...
      try {
        const upiUri =
          `upi://pay?pa=${encodeURIComponent(upiSettings.upiId)}` +
          `&pn=${encodeURIComponent(upiSettings.accountName || "Mahalakshmi")}` +
          `&am=${encodeURIComponent(amount.toFixed(2))}` +
          `&cu=INR` +
          `&tn=${encodeURIComponent("Mahalakshmi Jewellery Order")}`;
        ...
        if (!cancelled) {
          setDynamicUPIQR(qrDataUrl);
        }
      } catch (error) {
        console.error("Dynamic UPI QR Error:", error);
  ...
  }, [checkoutData, upiSettings]);

  // ==========================================
  // DIRECT UPI APP PAYMENT
  // Opens the customer's selected UPI app with
  // the exact order amount already filled in.
  // ==========================================
 ...
 const openUPIApp = (app: UPIApp) => {
  if (
    ...
    setUPIError(
      "Please select UPI and start the 5-minute payment session first."
    );
    return;
  }
  ...
  const urls: Record<UPIApp, string> = {
    GPay:
      `gpay://upi/pay?${params}`,
  ...
  // ==========================================
  //
  // Mobile UPI deep-links generally cannot
  // open from a laptop browser.
  //
  // Keep QR payment available instead.
  // ==========================================
  ...
  if (!isMobile) {
    setUPIError(
      "GPay, PhonePe and other UPI apps can be opened directly on a mobile device. On this laptop, please scan the QR code with your UPI app."
    );
  ...
  const handleUPIScreenshotChange = async (
  ...
  return;
    ...
    if (!upiSettings?.upiId) {
      setUPIError(
        "UPI payment details are not configured yet."
      );
      e.target.value = "";
      return;
    }
    ...
    setUPIProofValidationLoading(true);

    try {
      setProcessStep(
        "Analyzing payment screenshot and verifying amount, UPI ID and transaction details..."
      );
      ...
      if (!data?.success || !data?.validScreenshot) {
        throw new Error(
          data?.message ||
      ...
      setUPIProofValidated(true);
      setUPIError("");
      setProcessStep(
        "Payment proof verified successfully. You can now submit the order."
      );
    } catch (error: any) {
      console.error(
        "UPI Screenshot Validation Error:",
        error
      );
      ...
      setUPIError(
        error?.response?.data?.message ||
          error?.message ||
          "Payment proof validation failed. Please upload the actual successful UPI payment screenshot."
      );
    } finally {
      setUPIProofValidationLoading(false);
    }
  ...
  // ==========================================
  // UPI MANUAL PAYMENT - CREATE ORDER ONLY AFTER
  // THE SCREENSHOT HAS ALREADY PASSED VALIDATION.
  // ==========================================
  ...
  if (
    ...
    setUPIError(
      "The 5-minute payment session has expired. You will be returned to checkout."
    );
  ...
  if (
    ...
    setUPIError(
      "Please upload a valid successful UPI payment screenshot first."
    );
    return;
  }
  ...
  try {
    ...
    // first.
    //
    // The new backend endpoint performs:
    //
    ...
    // stock reduction
    //
    // atomically in the final flow.
    // ========================================

    setProcessStep(
      "Verifying your payment proof and creating your order..."
    );
    ...
    router.push(
      `/order-success?id=${data.order._id}&payment=pending`
    );
  } catch (error: any) {
    console.error(
      "UPI Payment Error:",
  ...
  <div
    className={`mt-4 inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold ${
      paymentSecondsLeft <= 60
        ...
        : "border-[#E8CFC6] bg-[#FFF4EF] text-[#8A6257]"
    }`}
  >
    <span
      ...
      .padStart(2, "0")}
                        ...
                        <div className="mt-5 grid gap-5 md:grid-cols-[220px_minmax(0,1fr)]">
                          <div className="rounded-2xl border border-[#E5DDD7] bg-white p-4">
                            {dynamicUPIQR || upiSettings.qrCode ? (
                              <div className="text-center">
                                <img
                                  src={dynamicUPIQR || upiSettings.qrCode}
                                  alt="UPI payment QR code"
                                  className="mx-auto aspect-square w-full max-w-[190px] object-contain"
                                />

                                <p className="mt-3 text-[10px] font-medium text-[#777]">
                                  Scan with GPay, PhonePe, Paytm or any UPI app
                                </p>

                                <p className="mt-1 text-[10px] font-semibold text-[#3A2528]">
                                  {dynamicUPIQR
                                    ? `Amount: ₹${payableAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
                                    : "Enter the exact order amount in your UPI app"}
                                </p>
                              </div>
                            ) : (
                          ...
                          <div className="min-w-0">
                            <div className="rounded-2xl border border-[#E8DFD9] bg-white p-4">
                              ...
                              {paymentExpiresAt && (
  <div className="mt-4 rounded-2xl border border-[#E8DFD9] bg-[#FFF8F5] p-4">
    <div className="flex items-center justify-between gap-3">
      <div>
        ...
        <p className="mt-1 text-xs text-[#6E6560]">
          Complete your UPI payment within 5 minutes.
        </p>
      </div>
      ...
      <div
        className={`rounded-xl px-4 py-2 text-lg font-bold tabular-nums ${
          paymentSecondsLeft <= 60
            ...
            : "bg-[#F8F0EC] text-[#8A6257]"
        }`}
      >
        {String(
          Math.floor(paymentSecondsLeft / 60)
        ).padStart(2, "0")}
        :
        {String(
          paymentSecondsLeft % 60
                            ...
                            ================================= */}
                            <div className="mt-4 rounded-2xl border border-[#E8DFD9] bg-white p-4">
                              <p className="text-xs font-semibold text-[#3E3734]">
                                Pay using your UPI app
                              </p>

                              <p className="mt-1 text-[11px] leading-5 text-[#777]">
                                Tap an app below to open it with the exact ₹{payableAmount.toLocaleString("en-IN")} amount.
                              </p>
                              ...
                              <div className="mt-4 grid grid-cols-2 gap-3">
                                <button
                                  ...
                                  disabled={loading || paymentSecondsLeft <= 0}
                                  className="flex min-h-12 items-center justify-center rounded-xl border border-[#E5DDD7] bg-white px-3 py-3 text-sm font-semibold text-[#3A2528] shadow-sm transition hover:border-[#C78B7B] hover:bg-[#FFF8F5] disabled:cursor-not-allowed disabled:opacity-50"
                                >
                                  GPay
                                </button>
                        ...
                        <div className="mt-5 rounded-2xl border border-[#E5DDD7] bg-white p-4">
                          ...
                          <input
                            ...
                            disabled={loading || upiUploading || upiProofValidationLoading}
                            className="hidden"
                          />
                          ...
                          {upiProofValidationLoading && (
                            <div className="mt-4 rounded-xl border border-[#E8DFD9] bg-[#FFF8F5] px-4 py-3 text-xs leading-5 text-[#8A6257]">
                              Analyzing screenshot... Please wait while we verify the payment status, amount, UPI ID and transaction reference.
                            </div>
                          )}

                          {upiProofValidated && !upiProofValidationLoading && (
                            <div className="mt-4 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-xs leading-5 text-green-700">
                              ✓ Payment proof verified successfully. The Submit Payment Proof button is now enabled.
                            </div>
                          )}
